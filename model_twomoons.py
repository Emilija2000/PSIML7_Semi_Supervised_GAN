...
class WN_ConvTranspose2d(nn.ConvTranspose2d):
    ...
    def forward(self, input, output_size=None):
        ...
        output_padding = self._output_padding(input, output_size, stride=self.stride, padding=self.padding, kernel_size=self.kernel_size)
...
class Discriminative(nn.Module):
    def __init__(self, config):
        ...
        elif config.dataset == 'two_moons':
            n_filter_1, n_filter_2 = 16, 32
        else:
...
class Discriminative(nn.Module):
    def __init__(self, config):
        ...
        # Assume X is of size [batch x 1 x 1 x 2]
        self.core_net = nn.Sequential(

            nn.Sequential(GaussianNoise(0.05), nn.Dropout2d(0.15)) if config.dataset == 'svhn' \
                else nn.Sequential(GaussianNoise(0.05), nn.Dropout2d(0.2)),

            WN_Conv2d(         1, n_filter_1, 1, 1, 1), nn.LeakyReLU(0.2),
            WN_Conv2d(n_filter_1, n_filter_1, 2, 1, 1), nn.LeakyReLU(0.2),
            WN_Conv2d(n_filter_1, n_filter_1, 2, 2, 1), nn.LeakyReLU(0.2),

            nn.Dropout2d(0.5) if config.dataset == 'svhn' else nn.Dropout(0.5),

            WN_Conv2d(n_filter_1, n_filter_2, 2, 1, 1), nn.LeakyReLU(0.2),
            WN_Conv2d(n_filter_2, n_filter_2, 2, 1, 1), nn.LeakyReLU(0.2),
            WN_Conv2d(n_filter_2, n_filter_2, 2, 2, 1), nn.LeakyReLU(0.2),

            nn.Dropout2d(0.5) if config.dataset == 'svhn' else nn.Dropout(0.5),

            WN_Conv2d(n_filter_2, n_filter_2, 2, 1, 0), nn.LeakyReLU(0.2),
            WN_Conv2d(n_filter_2, n_filter_2, 1, 1, 0), nn.LeakyReLU(0.2),
            WN_Conv2d(n_filter_2, n_filter_2, 1, 1, 0), nn.LeakyReLU(0.2),
...
class Generator(nn.Module):
    def __init__(self, image_size, noise_size=100, large=False):
        ...
        self.core_net = nn.Sequential(
            nn.Linear(self.noise_size, 64*4*6, bias=False), nn.BatchNorm1d(64*4*6), nn.ReLU(), 
            Expression(lambda tensor: tensor.view(tensor.size(0), 64, 4, 6)),
            nn.ConvTranspose2d(64, 32, kernel_size=2, stride=1, padding=1, bias=False), nn.BatchNorm2d(32), nn.ReLU(),
            WN_ConvTranspose2d(32, 1, kernel_size=(3,2), stride=1, padding=2, train_scale=True, init_stdv=0.1), nn.Tanh(),
        )
...
class Encoder(nn.Module):
    def __init__(self, image_size, noise_size=100, output_params=False):
        ...
        self.core_net = nn.Sequential(
            nn.Conv2d(  1, 16, 2, 1, 1, bias=False), nn.BatchNorm2d(16), nn.ReLU(),
            nn.Conv2d(16, 32, 2, 1, 1, bias=False), nn.BatchNorm2d(32), nn.ReLU(),
            nn.Conv2d(32, 64, 2, 1, 1, bias=False), nn.BatchNorm2d(64), nn.ReLU(),
            Expression(lambda tensor: tensor.view(tensor.size(0), 64*4*5)),
        )
        
        if output_params:
            self.core_net.add_module(str(len(self.core_net._modules)), WN_Linear(64*4*5, self.noise_size*2, train_scale=True, init_stdv=0.1))
            self.core_net.add_module(str(len(self.core_net._modules)), Expression(lambda x: torch.chunk(x, 2, 1)))
        else:
            self.core_net.add_module(str(len(self.core_net._modules)), WN_Linear(64*4*5, self.noise_size, train_scale=True, init_stdv=0.1))
