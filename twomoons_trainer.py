...
import data
import config
import model_twomoons as model
...
class Trainer(object):
    ...
    def __init__(self, config, args):
        # setting values of config attributes
        self.config = config
        ...
        # load data
        self.labeled_loader, self.unlabeled_loader, self.unlabeled_loader2, self.dev_loader, self.special_set, self.dataset = data.get_twomoons_loaders(config)
    ...
    def _train(self, labeled=None, vis=False):
        ...
        noise = torch.Tensor(unl_images.size(0), config.noise_size).uniform_().cuda()
        ...
        monitor_dict = OrderedDict([
                       ('unl acc' , unl_acc.data.item()), 
                       ('gen acc' , gen_acc.data.item()), 
                       ('max unl acc' , max_unl_acc.data.item()), 
                       ('max gen acc' , max_gen_acc.data.item()), 
                       ('lab loss' , lab_loss.data.item()),
                       ('unl loss' , unl_loss.data.item()),
                       ('fm loss' , fm_loss.data.item()),
                       ('vi loss' , vi_loss.data.item())
                   ])
    ...
    def eval_true_fake(self, data_loader, max_batch=None):
        ...
        for i, (images, _) in enumerate(data_loader.get_iter()):
            ...
            # true-fake accuracy
            unl_acc += torch.mean(nn.functional.sigmoid(unl_logsumexp).gt(0.5).float()).data.item()
            gen_acc += torch.mean(nn.functional.sigmoid(gen_logsumexp).gt(0.5).float()).data.item()
            # top-1 logit compared to 0: to verify Assumption (2) and (3)
            max_unl_acc += torch.mean(unl_logits.max(1)[0].gt(0.0).float()).data.item()
            max_gen_acc += torch.mean(gen_logits.max(1)[0].gt(0.0).float()).data.item()
    ...
    def eval(self, data_loader, max_batch=None, draw = False):
        self.gen.eval()
        self.dis.eval()
        self.enc.eval()
        ...
        for i, (images, labels) in enumerate(data_loader.get_iter()):
            ...
            loss += self.d_criterion(pred_prob, labels).data.item()
            cnt += 1
            incorrect += torch.ne(torch.max(pred_prob, 1)[1], labels).data.sum()
            if max_batch is not None and i >= max_batch - 1: break
            
            pred = torch.max(pred_prob, 1)[1]
            for i,v in enumerate(pred):
                if (v == 1):
                    data_1.append(images[i])
                else:
                    data_0.append(images[i])
        
        if draw:
            fig, ax = plt.subplots()
            x = [data_0[i][0].item() for i in range(len(data_0))]
            y = [data_0[i][1].item() for i in range(len(data_0))]
            ax.plot(x,y,'*',label=0)
            x = [data_1[i][0].item() for i in range(len(data_1))]
            y = [data_1[i][1].item() for i in range(len(data_1))]
            ax.plot(x,y,'*',label=1)
            plt.show()
    ...
    def visualize(self):
        ...
        vis_size = 100
        noise = torch.Tensor(vis_size, self.config.noise_size).uniform_().cuda()
        gen_images = self.gen(noise)
    ...
    def visualize_two_moons(self,gen_imgs):
        ...
        x = [gen_imgs[i][0][0][0].item() for i in range(len(gen_imgs))]
        y = [gen_imgs[i][0][0][1].item() for i in range(len(gen_imgs))]
        
        plt.plot(x,y,'ro',label=l)

    def visualize_acc_fig(self):
        x = [i for i in range(len(self.dis_lab_acc))]
        fig,ax = plt.subplots()
        ax.plot(x,self.dis_lab_acc,'b-')
        ax.plot(x,self.dis_unl_acc,'r-')
        ax.plot(x,self.dis_gen_acc,'g-')
        plt.show()
    ...
    def train(self):
        ...
        best_model = model.Discriminative(config)
        best_acc = 0
        ...
        while True:
            ...
            if iter % config.gen_samples_period == 0:
                noise = torch.Tensor(round(config.train_batch_size/2), config.noise_size).uniform_().cuda()
                gen_imgs = self.gen(noise)
                self.visualize_two_moons(gen_imgs)
              
            if iter % config.loss_graph_period == 0:
                self.visualize_acc_fig()

            if iter % config.eval_period == 0:
                train_loss, train_incorrect = self.eval(self.labeled_loader)
                draw = (iter % config.loss_graph_period == 0)
                dev_loss, dev_incorrect = self.eval(self.dev_loader, draw=draw)

                unl_acc, gen_acc, max_unl_acc, max_gen_acc = self.eval_true_fake(self.dev_loader, 10)

                train_incorrect = train_incorrect/(1.0 * len(self.labeled_loader))
                dev_incorrect =dev_incorrect /(1.0 * len(self.dev_loader))
                ...
                self.dis_lab_acc.append(1-dev_incorrect)
                self.dis_unl_acc.append(unl_acc)
                self.dis_gen_acc.append(gen_acc)
                
                if unl_acc>best_acc:
                    best_acc = unl_acc
                    best_model.load_state_dict(self.dis.state_dict())

            iter += 1
            self.iter_cnt += 1
        
        #with open('config_run1.txt', 'w') as file:
            #file.write(json.dumps(config))
            
        return best_model
        
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='twomoons_trainer.py')
    parser.add_argument('-suffix', default='run0', type=str, help="Suffix added to the save images.")

    args = parser.parse_args()

    trainer = Trainer(config.twomoons_config(), args)
    best_model = trainer.train()
    
    #torch.save(best_model, 'two_moons_run1.pt')
